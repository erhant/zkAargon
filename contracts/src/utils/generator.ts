...
/** Given items and their directions, computes the laser ins and outs. */
export function boardGenerate(
  items: ITEM[],
  itemDirs: DIR[]
): [number[][], number[][]] {
  if (items.length !== itemDirs.length) {
    throw new Error('array length mismatch');
  }
  const n = Math.floor(Math.sqrt(items.length));
  if (n * n != items.length) {
    throw new Error('array length must be square');
  }

  const ins: number[][] = Array.from({ length: items.length }, () =>
    Array(8).fill(0)
  );
  const outs: number[][] = Array.from({ length: items.length }, () =>
    Array(8).fill(0)
  );
  ...
  while (queue.length > 0) {
    ...
    const updatedCurOut = newOuts.map((value, index) =>
      value === 1 || outsCur[index] === 1 ? 1 : 0
    );
